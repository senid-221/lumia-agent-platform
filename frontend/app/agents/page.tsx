    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <div className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-3 text-2xl font-semibold tracking-tight">Hitamo agent wagufasha.</h1>
          <p className="mt-3 text-slate-600">Aba ni agents bari muri LUMIA platform kandi bafite status ya verified muri system.</p>
        </div>

        {loading && <div className="mt-4 text-sm text-slate-500">Loading available agents…</div>}
        {error && <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}

        {!loading && !error && agents.length === 0 && <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 text-sm text-slate-500">Nta verified agent uri available muri system ubu.</div>}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {agents.map((agent) => (
            <div key={agent.id} className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <h2 className="mt-4 text-lg font-semibold">{agent.displayName}</h2>
