...
const SERVICES = ['Hair Fashion','Driving Training','Restaurant Bookers','Shopping Orders','Car Repairing','Motor Repairing','Land Survey','Computer Repairing','Boutique Food Ordering','Website Building','Web Hosting','Teaching Tech','Prompt Generation','Flyer & Graphic Design','Jobs for Seekers'];
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <form onSubmit={submit} className="mt-7 space-y-5">
          ...
          <div><div className="mb-2 text-sm font-medium">Service ukora</div><div className="grid gap-2 sm:grid-cols-2">{SERVICES.map(service=><label key={service} className="flex cursor-pointer items-center gap-3 rounded-2xl border border-slate-200 p-3 text-sm hover:bg-slate-50"><input type="checkbox" checked={services.includes(service)} onChange={()=>toggleService(service)} className="h-4 w-4"/><span>{service}</span></label>)}</div></div>
