...
export default function AgentRequestsPage() {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <div className="mx-auto max-w-6xl px-5 py-10">
      <div><div className="text-sm font-medium text-slate-500">AGENT INBOX</div><h1 className="mt-2 text-4xl font-semibold tracking-tight">Service requests</h1><p className="mt-3 text-slate-600">Accept requests, contact customers, and update progress until the service is completed.</p></div>
      {loading&&<div className="mt-8 text-sm text-slate-500">Loading requests…</div>}
      {error&&<div className="mt-8 rounded-2xl bg-red-50 p-4 text-sm text-red-700">{error}</div>}
      <div className="mt-8 space-y-4">
        {!loading&&!error&&requests.length===0&&<div className="rounded-3xl border border-slate-200 bg-white p-8 text-sm text-slate-500">No assigned requests yet.</div>}
        {requests.map((r)=> <div key={r.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
